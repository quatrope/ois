...
class TestPSFCorrect(unittest.TestCase):
    ...
    def test_AdaptiveBramich_diffPSF(self):
        diff, opt, krn, bkg = ois.optimal_system(
            self.img, self.ref, method="AdaptiveBramich"
        )
        norm_diff = np.linalg.norm(diff) / np.linalg.norm(self.ref)
        # Assert it's a good subtraction
        self.assertLess(norm_diff, 1e-3)
...
class TestSmallRotationCorrect(unittest.TestCase):
    def setUp(self):
        ...
        self.ref = self.ref[
            (hr - h) // 2 + (hr - h) % 2 : -(hr - h) // 2 or None,
            (wr - w) // 2 + (wr - w) % 2 : -(wr - w) // 2 or None,
        ]

    def test_AdaptiveBramich_rotation(self):
        diff, opt, krn, bkg = ois.optimal_system(
            self.img, self.ref, method="AdaptiveBramich", poly_degree=2
        )
        norm_diff = np.linalg.norm(diff) / np.linalg.norm(self.ref)
        # Assert it's a good subtraction
        self.assertLess(norm_diff, 1e-3)
...
class TestMasking(unittest.TestCase):
    ...
    def test_Bramich_mask(self):
        diff, opt, krn, bkg = ois.optimal_system(
            self.img_masked, self.ref_masked, method="Bramich"
        )
        norm_diff = np.linalg.norm(diff.compressed()) / np.linalg.norm(
            self.ref_masked.compressed()
        )
    ...
    def test_AdaptiveBramich_mask(self):
        diff, opt, krn, bkg = ois.optimal_system(
            self.img_masked, self.ref_masked, method="AdaptiveBramich"
        )
        norm_diff = np.linalg.norm(diff.compressed()) / np.linalg.norm(
            self.ref_masked.compressed()
        )
...
class TestBackground(unittest.TestCase):
    ...
    def test_background_Bramich(self):
        diff, opt, krn, bkg = ois.optimal_system(
            self.img, self.ref, method="Bramich", bkgdegree=2
        )
        # Assert it's a good subtraction
        norm_diff = np.linalg.norm(diff) / np.linalg.norm(self.ref)
        self.assertLess(norm_diff, 1e-3)
    ...
    def test_background_AdaptiveBramich(self):
        diff, opt, krn, bkg = ois.optimal_system(
            self.img, self.ref, method="AdaptiveBramich", bkgdegree=2
        )
        # Assert it's a good subtraction
        norm_diff = np.linalg.norm(diff) / np.linalg.norm(self.ref)
        self.assertLess(norm_diff, 1e-3)
...
class TestVarConv(unittest.TestCase):
    def test_gen_matrix_system_sizes(self):
        ...
        mm, b = varconv.gen_matrix_system(
            image, refimage, 0, None, k_side, deg, bkg_deg
        )
    ...
    def test_convolve2d_adaptive_undoing(self):
        ...
        self.assertLess(
            np.linalg.norm(opt_ref - image, ord=np.inf)
            / np.linalg.norm(image, ord=np.inf),
            1e-8,
        )
        self.assertLess(
            np.linalg.norm((kernel - result_kernel).flatten(), ord=np.inf)
            / np.linalg.norm(kernel.flatten(), ord=np.inf),
            1e-8,
        )
    ...
    def test_eval_adpative_kernel(self):
        test_k = np.array([[[2.0], [3.0]], [[1.0], [4.0]]])
        self.assertLess(
            np.max(
                np.abs(
                    ois.eval_adpative_kernel(test_k, 0, 0)
        ...
        self.assertLess(
            np.max(
                np.abs(
                    ois.eval_adpative_kernel(test_k, 0, 1)
                    - np.array([[2.0, 3.0], [1.0, 4.0]])
                )
            ),
            1e-10,
        ...
        self.assertLess(
            np.max(
                np.abs(
                    ois.eval_adpative_kernel(test_k, 1, 0)
                    - np.array([[2.0, 3.0], [1.0, 4.0]])
                )
            ),
            1e-10,
        ...
        self.assertLess(
            np.max(
                np.abs(
                    ois.eval_adpative_kernel(test_k, 1, 1)
                    - np.array([[2.0, 3.0], [1.0, 4.0]])
                )
            ),
            1e-10,
    ...
    def test_convolve2d_adaptive_dtype_check(self):
        ...
        ois.convolve2d_adaptive(
            np.random.randint(low=0, high=100, size=(100, 100), dtype="int32"),
            kernel,
            0,
        )
